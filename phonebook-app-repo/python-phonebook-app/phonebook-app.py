...
# As we know, we will import necessary libraries.
...
# The hardest part of this project is to get endpoint of RDS instances.
# Since our RDS is created within cloudformation template, we need to get RDS endpoint and paste it here
# as environmental variable using Launch templates user data.
db_endpoint = open("/home/ec2-user/python-phonebook-app/dbserver.endpointfile", 'r', encoding='UTF-8') 

# Configure mysql database

# Once we are done with the database, we are going to create database.
# we need to configure our database. Lets have a look at these configuration.
app.config['MYSQL_DATABASE_HOST'] = db_endpoint.readline().strip()
app.config['MYSQL_DATABASE_USER'] = 'admin'
app.config['MYSQL_DATABASE_PASSWORD'] = 'Franklin_123'
...
# Lets paste Because of the id is auto_incremental, I don't need to worry about to id column.
# mysql is going to give id on behalf of us.
...
# and returns result as list of dictionary `[{'id': 1, 'name':'XXXX', 'number': 'XXXXXX'}]`.

# This function is to find my results that has "keyword" into database
def find_persons(keyword):
    # You are very familiar with this query. This query will select all columns where the name like keyword.
    # strip will remove all the white spaces, and lower will turn uppercase into lowercase.
    query = f"""
